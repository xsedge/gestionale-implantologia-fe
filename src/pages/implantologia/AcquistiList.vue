<template>
  <q-page class="q-pa-md implantologia-page">
    <q-card flat bordered class="list-card">
      ...
      <q-card-section>
        <q-table :rows="filteredAcquisti" :columns="columns" row-key="id" flat bordered :loading="store.loading"
          no-data-label="Nessun acquisto registrato" rows-per-page-label="Acquisti per pagina" :pagination="pagination">
...
function calcolaTotale(acquisto) {
  ...
  return acquisto.dettagli.reduce((sum, det) => sum + (Number(det.totaleRiga) || (Number(det.prezzoUnitario) || 0) * det.quantita), 0)
}
